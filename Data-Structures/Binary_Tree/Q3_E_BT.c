...
int countOneChildNodes(BTNode *node) {
    ...
    else if (node->left != NULL && node->right == NULL) count = 1;

    int l_result = countOneChildNodes(node->left);
    int r_result = countOneChildNodes(node->right);
    
    return l_result + r_result + count;
}
